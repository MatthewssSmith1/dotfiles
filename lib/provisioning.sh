...
validate_provisioning_manifest() {
  ...
  jq -e '.schema_version == 1 and .status == "accepted-host-owned-assistant-lifecycle" and
    ...
    .supersedes == "manifests/proposals/2026-07-17-stage5-tool-pins.json" and
...
check_omarchy_neovim_drift() {
  ...
  package="$(jq -er '.artifacts[] | select(.id == "omarchy-nvim-lazy-lock") | .release' "$DOTFILES_DIR/manifests/sources.json")" || {
    log 'error: active source manifest has no accepted omarchy-nvim release'; return 1;
  }
