#!/usr/bin/env bash
# Neovim area: lazy-lock restore transaction, rollbacks, flock, XDG runtime migration, reviewed links, ready dispatch, readiness rows, and isolated real-startup runtime policy.
...
bootstrap_repo="$(copy_repo_fixture bootstrap)"
...
# the real Neovim startup below.
unset DOTFILES_NVIM_LAZYVIM_REPOSITORY
...
install_network_sentinels "$home" with-git
...
[[ ! -e "$home/network-attempted" ]] || fail 'ordinary restored startup attempted a network-capable command'
