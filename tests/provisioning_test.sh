#!/usr/bin/env bash
# Retained-tool provisioning via bootstrap --provision and lib/provisioning.sh.
...
# Local invoke_fixture/capture: provisioning runs need XDG roots, fixture
# artifact env, DOTFILES_TEST_ARCH, and a fixture-first PATH that the harness
# capture helper does not provide.
invoke_fixture() {
  local home="$1"; shift
...
pid=$!
set -e
wait_for_file "$hold/after-provisioning-receipt-validation-read.ready"
...
pid=$!
set -e
wait_for_file "$hold/provisioning-tool-after-commit-before-cleanup.ready"
...
pid=$!
set -e
wait_for_file "$hold/provisioning-tool-after-combined-receipt.ready"
...
pid=$!
set -e
wait_for_file "$hold/provisioning-tool-before-commit.ready"
...
pid=$!
set -e
wait_for_file "$hold/provisioning-mise-after-download.ready"
...
pid=$!
set -e
wait_for_file "$hold/provisioning-tool-after-staging.ready"
...
printf 'PASS: %d retained-tool provisioning test groups\n' "$TEST_COUNT"
