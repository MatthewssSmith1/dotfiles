#!/usr/bin/env bash
# Real tmux parser compatibility gate (opt-in gate, run manually; see README).
# Requires a prepared --fixture-root cache, so it is deliberately named without
# the _test.sh suffix and the default runner glob skips it.
