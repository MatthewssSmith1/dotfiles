...
Bash with Starship is the evaluated primary shell during the migration, while the existing zsh configuration remains behaviorally frozen as an escape hatch (see [Shell](tools/shell.md)). After the migration, decide deliberately whether to converge the two setups or retire zsh. Until that decision, zsh receives no new features and no reconciliation with Bash.
