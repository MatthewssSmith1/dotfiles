...
Ubuntu 24.04 and newer, including remote VPSs and WSL Ubuntu, uses the `ubuntu` profile: pinned Omarchy baseline snapshots plus portability adapters. See [Architecture](../architecture.md).
...
- WSL Ubuntu 24.04+: `ubuntu` profile. Other WSL distributions and Omarchy on WSL are unsupported.
- When connecting to any of these from Windows Terminal (including SSH into a VPS), the client-side guidance in [Windows Terminal](windows-terminal.md) applies — tmux runs on the host, but the keys are intercepted or encoded by the client terminal.

## Manual Package Step

On WSL, keep the checkout in the Linux filesystem (for example `~/dotfiles`),
and install the dependencies inside Ubuntu. Windows Terminal settings remain
...
Optional child-only injection from persistent, host-owned environment bundles is documented in [Local environment bundles](../tools/secrets.md). The launcher is offline and read-only; dotfiles does not create, inspect, or remove real bundles or application wrappers.
...
with Tokyo Night and relative line numbers.
The optional OpenCode area also applied and passed its check, with the native
OpenCode 1.18.29 user installation supplying the runtime. Desktop is
validation-only and deployed no configuration on this headless host.

This validates the Linux user configuration. Windows Terminal key handling,
font rendering, clipboard integration, and optional service startup were not
...
- Pre-cutover Node 24.18.0, pnpm 11.13.1, Claude Code 2.1.212, Worktrunk 0.68.0, tmux, Neovim 0.12.4, and Starship 1.26.0.
- Offline area checks and two ordinary configuration-only applies.
- Bash interactive startup, Git identity and credential layering, an isolated and default tmux server, pinned Neovim plugin restore, and headless `:checkhealth`.
