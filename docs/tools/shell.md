...
refresh recovery, host-local sourcing, and apply/check/remove behavior using
fake Stow and isolated native/Ubuntu homes. `tests/secrets_test.sh` is the
canonical `dotfiles-secret` contract.
