...
zsh is the current login shell and remains available as a behaviorally frozen escape hatch while the stock Omarchy Bash experience is evaluated. History, vi mode, key bindings, Powerlevel10k, Zinit, plugins, completion styles, zoxide, fzf, mise, Worktrunk, and aliases receive no new features during the migration. Its existing OpenCode path is minimally reordered ahead of inherited mise paths so the native owner wins. Converging the two setups or retiring zsh is a deliberate post-migration decision recorded in [Deferred Work](../deferred.md#shell-convergence).
