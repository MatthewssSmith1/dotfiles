...
The policy records onboarding as complete and disables version checks because
the runtime is repository- and mise-pinned. Agent-detection manifest checks are
intentionally enabled and may use the network during ordinary Herdr runtime.
Changing settings through Herdr writes through the Stow link into this checkout;
edit reviewed preferences in the repository and keep the exact policy derivation.
